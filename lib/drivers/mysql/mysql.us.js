...
MySql.prototype.summaryFetch = function(summaryname, query, partition, readers, callback) {
	...
	me._fetchSummaryDataObject(summaryname, function (err, doSummary) {
		if (err) {
			...
			var orderedCols = _.where(doSummary.obj.columns, {ordered: "1"});
			var strWhere = '';
 ...
 * @param  {Function}	callback   Classical function(err, data) where data is the passed dataObject
 */
MySql.prototype._deleteDataObject = function (dataObject, callback) {
	var arrQueries = [];
	arrQueries.push(mysql.format("DELETE FROM dataobject WHERE uuid = ?", [dataObject._id]));
	...
	this._db.query(arrQueries.join(';'), [], function (err, result) {
			if (err) {
				callback(err, result);
			} else {
				callback(null, dataObject);
